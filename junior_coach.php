<?php
	...
	$head_coach = mysql_query ("SELECT * FROM coachs WHERE team=2 		
							AND position='Старший тренер'", $db);
	if (!$head_coach) {
	...
	$assistant_coach = mysql_query ("SELECT * FROM coachs WHERE team=2 		
							AND position='Асистент тренера'", $db);
	...
	$goalkeeper_coach = mysql_query ("SELECT * FROM coachs WHERE team=2 		
	...
	$physical_coach = mysql_query ("SELECT * FROM coachs WHERE team=2 		
							AND position='Начальник команди'", $db);
