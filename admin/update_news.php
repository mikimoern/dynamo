			...
			<div id="center">
				<?php
					if ( isset($title) && isset($date) && isset($image) && isset($big_img) && isset($description) && isset($text) ) {
						$result = mysql_query ("UPDATE news SET title='$title', date='$date', image='$image', big_img='$big_img',
												description='$description', text='$text' WHERE id='$id'", $db);
						if ($result == 'true') {
							echo "<p>Новина була оновлена</p>";
						}
						else {
							echo "<p>Новина не була оновлена</p>";
						}
					}
					else  {
